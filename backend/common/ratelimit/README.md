...
いずれも障害は NFR-08（外部依存失敗=ERROR）でログ化する。「アラート」はこの ERROR ログによる可観測化を指し、別途の通知機構（ページャ等）は POC 範囲外。
...
本番ではエッジ（WAF / API ゲートウェイ）のレート制限が一次防御であり、本基盤（アプリ層）は**二次防御**。POC はアプリ層のみを実装する。
