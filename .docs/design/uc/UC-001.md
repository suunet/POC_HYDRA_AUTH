...
| 主アクター | —（人のアクターなし。呼び出し元は EXT-04（死活監視ツール）） |
| 副アクター（任意） | — |
...
EXT-04（死活監視ツール: ロードバランサ・Kubernetes等）や開発者が、app-service が稼働していることを確認する（BUC-S01・FR-20・NFR-04）。
...
|---|---|
| `GET /health`（死活監視の受口・SCR-18） | EXT-04（死活監視ツール） |
...
```mermaid
sequenceDiagram
  participant Caller as EXT-04 死活監視ツール
  participant Router as app-service ルーター
  participant UC as UC-001 ヘルスチェック ユースケース
  participant Log as ログ基盤 （NFR-09）
