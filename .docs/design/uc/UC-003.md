...
評価は基本フローのステップ順（**E4=レートが最初**）。形式不正（ボディ欠落・非JSON・`token`欠落/空）は本評価順の対象外で `validation-error`（400）を返す（レートも消費しない）。**E1a/E1b/E2間は状態秘匿性のため以下の順で判定する**（不在・使用済みを期限判定より先に倒し、「トークンが実在した」ことを漏らさない）。
...
| E1a: トークンが存在しない（ステップ3・ハッシュ一致なし） | 400 Bad Request・`application/problem+json`・`type: .../invalid-token`。WARNINGログ（`ctx: "email_verification"`）。**論理削除済みユーザーのトークンも同一応答**（存在を漏らさない・ステータス更新0行で検知） |
...
| Handler / UseCase / Job | Handler: `backend/auth/api/http/handler.go`（`VerifyEmail`）／ UseCase: `backend/auth/app/command/verify_email.go`（`VerifyEmailHandler.Handle`）／ Domain: `backend/auth/domain/verification.go`／ Repository: `backend/auth/adapters/db/email_confirmation_tokens_repo.go`＋`users_repo.go`／ RateLimiter: `backend/auth/adapters/ratelimit/emailverify.go`（基盤: `backend/common/ratelimit/countingwindow.go`・`hasher.go`） |
| テスト | `backend/tests/unit/auth_verify_test.go`／ `backend/tests/component/auth_verify_test.go`・`ratelimit_test.go`（`UC-003` でgrep可・実DB/実Redis） |
| 外部連携 | EXT-02（Redis・IPレート記録） |
| 設定・フラグ | `AUTH_PORT`・`POSTGRES_URL`・`REDIS_ADDR`・`RATELIMIT_HMAC_SECRET`（`cmd/auth/main.go`。ホスト公開ポートはcomposeの`AUTH_SERVICE_PORT`） |
