...
| INF-13（登録送信記録・本チケットで発番） | 両方（レート確認の読み・記録の書き。Redis TTL 5分） |
...
| VAR-16（登録レートリミット・本チケットで発番） | 同一メールアドレス5分に1回。**登録済み/未登録に関わらず一様に記録・判定**（判別不能性の維持） |

## 3. 主成功シナリオ（基本コース）

1. [アクター] メールアドレスとパスワードを送信する（SCR-01: `POST /auth/register`）
2. [システム] 登録送信記録（INF-13）を確認し、VAR-16（登録レートリミット）を超過していないことを判定して記録する（登録済み/未登録に関わらず一様に行う）
...
| E4: レートリミット超過（ステップ2・VAR-16） | 429 Too Many Requests・`application/problem+json`・`retry_after` を含める。**登録済み/未登録に関わらず同一挙動**（A1の判別不能性を429で迂回させない）。WARNINGログ |
...
sequenceDiagram
  ...
  API-->>User: 429 problem+json (retry_after)
  end
...
| ルーティング | `backend/auth/api/http/`（openapi.yaml から生成される登録関数。P5仕上げサイクルで実パス化） |
| Handler / UseCase / Job | `backend/auth/api/http/`（strict-server実装）／ `backend/auth/app/command/`（P5仕上げサイクルで実パス化） |
| テスト | `backend/tests/unit/`・`backend/tests/component/`（`UC-002` でgrep可。P5仕上げサイクルで実パス化） |
| 外部連携 | EXT-01（メールサーバー・SMTPアダプタ）・EXT-02（Redis・レート記録） |
| 設定・フラグ | `AUTH_SERVICE_PORT`・`POSTGRES_URL`・SMTP接続先・Redis接続先（環境変数。P5で確定） |
