...
## 8. 実装参照（突合用・T-008 P5で実パス確定）

| 種別 | 参照 |
|---|---|
| HTTP | `POST /auth/email-verify/resend`（SCR-03） |
| ルーティング | `backend/auth/api/http/openapi.yaml`（`resendEmailVerification`）→ `openapi.gen.go`/`client/client.gen.go` 生成・`backend/auth/module.go`（`ResendLimiter` 配線） |
| Handler | `backend/auth/api/http/handler.go`（`ResendEmailVerification`・200/429/503マッピング） |
| UseCase | `backend/auth/app/command/resend_email_verification.go`（`ResendEmailVerificationHandler`・レート最初→状態確認→既存無効化＋新発行＋送信） |
| RateLimiter | `backend/auth/adapters/ratelimit/resend.go`（`NewResendEmailLimiter`・メールキー`FixedWindowLimiter`・fail-open） |
| Repository | `backend/auth/adapters/db/email_confirmation_tokens_repo.go`（`ReissueEmailConfirmationToken`・一括無効化＋新発行の単一Tx）・`users_repo.go`（`FindUserByEmail`）。sqlc: `InvalidateActiveEmailConfirmationTokensByUser`（`queries/email_confirmation_tokens.sql`） |
| テスト | `backend/tests/unit/auth_resend_test.go`・`backend/tests/component/auth_resend_test.go`（`UC-004` でgrep可） |
